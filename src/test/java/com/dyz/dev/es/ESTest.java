package com.dyz.dev.es;

import com.dyz.dev.DevApplicationTests;
...
public class ESTest extends DevApplicationTests {
    @Test
...
//        QueryBuilders.matchQuery("name", "Robert");
        MatchQueryBuilder matchQueryBuilder = QueryBuilders.matchQuery("content", "讲师，清华")
                .analyzer("ik_max_word");

        BoolQueryBuilder builder = QueryBuilders.boolQuery().must(matchQueryBuilder).must(rangeQueryBuilder);
        SearchSourceBuilder sourceBuilder = new SearchSourceBuilder();
        sourceBuilder.query(builder)
                .highlighter(new HighlightBuilder().field("content").preTags("<span>").postTags("</span>"));
